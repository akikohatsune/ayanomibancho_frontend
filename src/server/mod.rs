...
pub fn build_web_router(state: AppState) -> Router {
    Router::new()
        ...
        .route("/friends", get(frontend::friends_page))
        .route("/api/friends", get(frontend::list_friends_api))
        .route("/api/friends/add", post(frontend::add_friend_api))
        .route("/api/friends/remove", post(frontend::remove_friend_api))
